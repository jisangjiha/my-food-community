...
6. **기록은 멱등하다.** 같은 결제 건이 몇 번 들어와도 결제 행은 하나다.
...
두 문이 같은 검증(§5)과 같은 기록(§4.1)을 공유한다.  성공 → 303 /payments/[paymentId] · 실패 → 303 /payments/failed?reason=…
```

두 경로는 **소유자를 정하는 방법만** 다르다. 갈라 두면 한쪽에만 검사를 추가하는 날이 온다.
...
- 웹훅이 알려 주는 것은 `paymentId`까지다. **상태는 조회 API로 다시 확인한다**(§5).
...
| `product_id`, `user_id` | 소유자는 §4.2가 정한다 |
...
- 모든 쓰기는 `security definer` 함수를 지난다. 현재는 `record_payment` 하나다. 중복
  확인 → 스냅샷 → 결제 행을 한 트랜잭션에서 처리하고, unique 위반은 안에서 잡아 기존
  행 id를 돌려준다(스냅샷도 함께 롤백되어 고아가 남지 않는다).
...
**의도된 경고 두 건**(고치지 않는다): `rls_enabled_no_policy` / `payment_snapshot`,
`authenticated_security_definer_function_executable` / `record_payment`.

## 5. 검증

리다이렉트든 웹훅이든 **같은 검증**을 지난다. 하나라도 어긋나면 기록하지 않는다.
...
| `unavailable` | 포트원 조회 실패, DB 기록 실패 | 의미 있음 | `500` |
...
문자열은 `payments/dto.ts`의 상수에서만, 금액 표기는 `products/format.ts`의 `formatWon` 하나.
...
## 8. 아직 구현하지 않은 것 — §1의 절대 규칙과 §3.2가 그대로 적용된다

### 8.1 결제 취소·환불 (취소 웹훅 포함)

- 취소는 **서버에서 포트원 취소 API를 부른 뒤** 그 결과로 기록한다. DB만 바꾸고
  포트원에 알리지 않는 "취소"를 만들지 않는다.
- 취소 금액도 §1-1을 따른다. 환불 비율은 서버가 계산하고, 규정(기간별 비율)은 TS 상수 한
  곳에 두고 화면들이 그것을 읽는다.
- `type = 'CANCEL'`, `amount`는 **음수**, `transaction_key`는 **원 결제와 같은 값**.
  원 결제 행을 지우거나 덮어쓰지 않는다(§1-7).
- 취소 쓰기도 새 `security definer` 함수를 지난다(§4.1). 함수 안에서 소유권과
  `그룹 합계 − 취소액 ≥ 0`을 다시 확인한다.
- 취소 웹훅(`Transaction.Cancelled` / `Transaction.PartialCancelled`)은 §3.2 그대로다 —
  서명 검증 → raw body → **본문이 아니라 조회 API로 상태 재확인** → 멱등 → 200 전에 처리
  완료. 멱등 키는 `cancellationId`다 — `transaction_key`만으로는 취소 행을 구분할 수
  없다(부분 취소가 여러 번일 수 있다).
- 취소도 **웹훅과 사용자 요청 두 문**이 생긴다. 검증과 기록은 결제와 똑같이 한 벌로
  공유한다. 부분 취소 전에는 **전액 취소만** — 반쯤 지원하는 코드가 가장 위험하다.

### 8.2 범위 밖

가상계좌(`VIRTUAL_ACCOUNT_ISSUED`), 정기결제·빌링키, 인원 선택·수량, 중복 구매 방지,
에스크로, 현금영수증. 붙일 때는 이 문서에 먼저 규칙을 적고 시작한다.

## 9. 확인

- `npx tsc --noEmit` · `npm run lint` · `npm run build`
- 마이그레이션을 했다면 `get_advisors({ type: "security" })` — 의도된 두 건 외에 새 경고 없음
- 손으로 밟아 볼 흐름: 정상 결제 → 완료 화면 **새로고침**(행이 하나인지) → 콘솔에서 **웹훅
  재전송**(그래도 하나인지) → 결제창에서 취소 → 비로그인으로 결제 버튼 → 남의
