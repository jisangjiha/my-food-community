...
| 경우 | 처리 |
| --- | --- |
| 키 없음 | `env.ts`가 던진다. Supabase와 같다 — 검색이 이 화면의 존재 이유라 조용히 넘어갈 수 없다 |
...
발급에는 NCP 콘솔 로그인이 필요하다. 없으면 `/register/place/search`가 500이
된다. 지도와 달리 폴백을 두지 않는 이유는 위 "에러 처리" 표의 첫 줄이다 —
지도는 등록 화면의 한 조각이지만, 검색은 검색 화면의 존재 이유다.
...
3. 키 없이 `/register/place/search?q=오월` → 500이 나고 서버 로그에 환경변수
   안내가 찍힌다
