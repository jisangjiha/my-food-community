...
- **결제 버튼 높이 50 → 48** — 버튼 스케일은 32/40/48뿐이다. 스케일 밖 크기를
  만들지 않는다.
- **요일은 실제 날짜에서 나온다** — 시안의 `12/14 토`는 목업 값이다. 2026-12-14는
  월요일이므로 화면에는 `12/14 월`이 뜬다. 날짜를 시안에 맞추려고 요일을
  하드코딩하지 않는다.
- **배너 태그 문구** — 시안은 `이번 주 모임` 고정이다. 상품마다 다른 문구를 박을 수
  없어 `category_label`의 앞 조각(`이웃 모임`)을 쓴다. 뒤 조각은 상세의 눈썹에서 보인다.
- **배너 높이 120 고정, 내용은 위아래로 분배** — 시안은 내용을 위에 붙이고 아래
  27px를 비워 둔다. `justify-between`이 같은 배치를 만들고 폭이 커져도 유지된다.
