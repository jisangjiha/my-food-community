 ...
 * `?place=`로 어떤 장소가 선택됐는지 받는다. 없으면 시안이 그린 오월식당을 보여 준다.
 *
...
export default async function PlaceRegisterPage(
  ...
  const { place } = await props.searchParams;
  const selected =
    findPlace(typeof place === "string" ? place : null) ?? DEFAULT_PLACE;
      ...
      >
        ...
        */}
        ...
        <Card>
          ...
          <h2 className="type-heading-md text-text-default">{selected.name}</h2>
          <p className="type-body-lg text-text-muted">
            {placeAddress(selected)}
          </p>
        </Card>
